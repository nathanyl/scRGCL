...
from model import (scRGCL,edge_transform_func)
...
def get_parser(parser=None):
    ...
    parser.add_argument('-in','--infile',type=str,)
     parser.add_argument('-out-dir','--outdir',type=str,default='../results')
...
def train2(model,optimizer,train_loader,epoch,device,loss_fn =None,scheduler =None,verbose=False  ):
    ...
    for idx,data in enumerate( train_loader):
        ...
        loss.backward()
        # c_loss_all += c_loss.item() * data.num_graphs
...
def test2(model,loader,predicts=False):
    ...
    for data in loader:
        ...
        # print(data.y.shape)
        cell = torch.squeeze(data.x)
        cell = cell.cpu().data.numpy()
        cell = cell.T
        if len(cell_all) == 0:
            cell_all = cell
        else:
            cell_all = np.vstack((cell_all, cell))
    ...
    if predicts:
         return acc,f1,y_true,np.array(y_pred),y_output,cell_all
    else:
        return acc,f1
...
if __name__ == '__main__':
    ...
    model = scRGCL(in_channel=dataset.num_expr_feaure, num_nodes=gene_num,
    ...
    scheduler = CosineAnnealingWarmRestarts(optimizer, 2, 2, eta_min=int(min_lr), last_epoch=-1)
    max_metric = float(0)
    max_metric_count = 0
    weights_list = []
    ...
    #valid_cells = valid_dataset.Expr.copy()
    #valid_cells.to_csv('train_cells1.csv', index=False)
    if True:
        # add more samples for those small celltypes
        train_dataset.duplicate_minor_types(dup_odds=50)
    ...
    # samples_weight = samples_weight.double()
    # #from torch.utils.data import WeightedRandomSampler
    # sampler1 = WeightedRandomSampler(samples_weight, len(samples_weight), replacement=True)
    #
    train_loader = DataLoader(train_dataset, batch_size=batch_size,num_workers=num_workers, shuffle=True,collate_fn = collate_func,drop_last=True)
    ...
    test_acc,test_f1,y_true,y_pred,y_output,cell_all = test2(model,test_loader,predicts=True)
    ...
    # data1 = pd.DataFrame(cell_all)
    # data1 = data1.applymap(lambda x: 0 if isinstance(x, float) and abs(x) < 1e-5 else x)
    # data1.to_csv('16_test.csv', header=None, index=None)